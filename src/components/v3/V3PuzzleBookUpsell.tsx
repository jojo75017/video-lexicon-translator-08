...
import { X, Loader2, Puzzle, Lock, Crown } from 'lucide-react';
import { getStripe, getStripeEnvironment } from '@/lib/stripe';
import { supabase } from '@/integrations/supabase/client';
import { toast } from 'sonner';

/**
 * Modal d'upsell du Générateur de Livres de Jeux & Énigmes.
 * Deux portes d'entrée : achat one-shot 27 € OU passage au plan Pro (inclus).
 */
const V3PuzzleBookUpsell: React.FC<{ defaultEmail?: string; onClose: () => void }> = ({
  defaultEmail = '',
  onClose,
}) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('v3-upsell-checkout', {
        body: {
          ...
          returnUrl: `${window.location.origin}/v3/livres/jeux-enigmes?paiement=ok&session_id={CHECKOUT_SESSION_ID}`,
        },
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Débloquez le Générateur de Livres d'Énigmes et Jeux KDP"
      >
        <button
          onClick={onClose}
          aria-label="Fermer"
        ...
        {!clientSecret ? (
          <>
            ...
            >
              <Puzzle className="h-7 w-7" style={{ color: 'var(--v3-emerald, #064e3b)' }} />
            </div>
            ...
            >
              Débloquez le Générateur de Livres d'Énigmes &amp; Jeux KDP
            </h3>
            <p className="text-sm text-center mb-5" style={{ color: 'var(--v3-muted, #6b7280)' }}>
              Créez des livres d'activités, de casse-têtes et d'énigmes prêts à publier sur Amazon
              KDP en quelques clics.
            </p>
...
export default V3PuzzleBookUpsell;
