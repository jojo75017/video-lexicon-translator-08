...
export default function IllustrationGeneratorPanel({
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!data?.illustrationPath) throw new Error('Aucune image renvoyée.');

      await onGenerated(data.illustrationPath as string);
      await refresh();
      toast.success(
        data.funding === 'ebookstudio'
          ? `Illustration ajoutée. Générations incluses restantes : ${data.credits?.remaining ?? 0}.`
          : 'Illustration ajoutée avec votre clé personnelle.',
      );
    } catch (err) {
      const message =
        err instanceof Error ? err.message : 'Génération impossible pour le moment.';
      setError(message);
      toast.error(message);
    } finally {
      setBusy(false);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !busy && setOpen(v)}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Illustration IA (sans aucun texte)</DialogTitle>
          <DialogDescription>
        ...
        {!hasAccess && !loading ? (
          <div className="space-y-3 text-sm">
            <p className="rounded-lg border border-border bg-muted/40 p-3">
              ...
              unique). Les outils de texte restent utilisables sans achat.
            </p>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="ill-summary">Sujet du livre</Label>
              <Textarea
                id="ill-summary"
                rows={3}
                value={summary}
            ...
            <DialogFooter className="gap-2 sm:justify-between">
              ...
              <Button onClick={generate} disabled={busy || noFunding}>
                {busy ? (
                  <>
                  ...
                  <>
                    <Sparkles className="mr-2 h-4 w-4" /> Générer l’image
                  </>
                )}
              </Button>
            </DialogFooter>
