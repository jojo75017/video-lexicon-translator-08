...
import { CloudUpload, RefreshCw, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
const SystemeIoSyncPanel = () => {
  ...
  const launchSync = async () => {
    ...
    if (data?.already_running) {
      ...
      toast.success("Synchronisation Systeme.io lancée en arrière-plan");
    }
    fetchStatus();
  };
    ...
    <Card className="bg-card border-border">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          Pousse prospects, leads et clients vers Systeme.io. Les désabonnés ne sont jamais envoyés.
      ...
      <CardContent className="space-y-3">
        ...
        {log && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {log.state === 'done'
              ? `Dernière synchro terminée — ${log.synced} réussis, ${log.failed} échecs`
