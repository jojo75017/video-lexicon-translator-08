...
const EbookPlannerPage: React.FC<EbookPlannerPageProps> = ({
  ...
  // Sauvegarde forcée périodique toutes les 60 secondes (filet de sécurité)
  useEffect(() => {
    const forceSaveInterval = setInterval(async () => {
      ...
      if (data.ebookTitle) {
        try {
          ...
          console.log('🔄 [Auto-save périodique] Projet sauvegardé en base');
        } catch (err) {
          console.error('❌ [Auto-save périodique] Erreur:', err);
        }
      }
    }, 60000);
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      ...
      if (data.ebookTitle) {
        ...
        // Utilise sendBeacon pour sauvegarder même si la page se ferme
        try {
          const serialized = JSON.stringify(projectData);
          localStorage.setItem('ebook_pending_save', serialized);
          console.log('💾 [beforeunload] Données mises en cache pour sauvegarde');
        } catch {}
      }
    };
  ...
  useEffect(() => {
    const pendingSave = localStorage.getItem('ebook_pending_save');
    if (pendingSave) {
      try {
        const projectData = JSON.parse(pendingSave);
        localStorage.removeItem('ebook_pending_save');
        saveProject(projectData, false).then(() => {
          console.log('✅ [Recovery] Sauvegarde en attente récupérée');
        });
      } catch {}
    }
  }, []);
