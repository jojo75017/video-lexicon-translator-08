...
const InternalLinksAnalyzer = () => {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [results, setResults] = useState<LinkAnalysisData | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const mockResults: LinkAnalysisData = {
        totalPages: 28,
        totalLinks: 187,
        orphanedPages: 4,
        averageDepth: 2.6,
        ...
        recommendations: [
          ...
          "Réduire la profondeur moyenne des pages"
        ]
      };
...
export default InternalLinksAnalyzer;
