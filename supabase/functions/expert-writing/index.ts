...
serve(async (req) => {
  ...
  try {
    const { chapterTitle, chapterContext, targetAudience, expertise, userApiKey} = await req.json();
...
- Micro-cliffhanger en fin de chapitre`;
    ...
    const content = await callGemini(userApiKey, systemPrompt, userPrompt, { maxTokens: 8000, temperature: 0.8, timeout: 120000 });
