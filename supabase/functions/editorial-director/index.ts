...
async function callGemini(systemPrompt: string, userPrompt: string, opts: { maxTokens?: number; temperature?: number; timeout?: number } = {}) {
  ...
  const response = await fetch(
    ...
    { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ system_instruction: { parts: [{ text: systemPrompt }] }, contents: [{ role: "user", parts: [{ text: userPrompt }] }], generationConfig: { temperature: opts.temperature ?? 0.7, maxOutputTokens: opts.maxTokens ?? 2000 } }), signal: controller.signal }
  );
  clearTimeout(timeoutId);
  if (!response.ok) { const errText = await response.text(); console.error("Gemini error:", response.status, errText); if (response.status === 429) throw { status: 429, message: "Limite de requêtes Gemini atteinte." }; throw new Error(`Erreur Gemini: ${response.status}`); }
...
Fournis une analyse complète et professionnelle en JSON.`;
    ...
    console.log("Gemini 3 Flash: editorial-director pour:", sujet);

    const content = await callGemini(systemPrompt, userPrompt, { maxTokens: 2500 });
    ...
    const errorMessage = error.name === 'AbortError' ? 'Timeout - analyse trop longue' : (error.message || "Erreur inconnue");
