...
Le score overall_score doit être entre 0 et 100 (100 = parfaitement cohérent).`;
    ...
    // Handle series-bible generation (uses Lovable AI - no API key needed)
    if (type === 'series-bible') {
      console.log('Processing series bible generation...');
      const LOVABLE_API_KEY = Deno.env.get('LOVABLE_API_KEY');
      ...
      const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'google/gemini-2.5-flash',
          messages: [
            { role: 'system', content: 'Tu es un expert en création littéraire et planification de séries de livres. Tu génères des bibles de série détaillées et cohérentes en JSON valide.' },
            { role: 'user', content: prompt }
          ],
        }),
      });
      ...
      if (!response.ok) {
        ...
        console.error('Lovable AI error:', errorText);
        return new Response(
          JSON.stringify({ error: 'Erreur lors de la génération de la bible de série' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (type === 'next-tome') {
      ...
      const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'google/gemini-2.5-flash',
          messages: [
            { role: 'system', content: 'Tu es un expert en création littéraire. Tu génères des plans de tomes détaillés et cohérents avec la série existante. Réponds en JSON valide.' },
            { role: 'user', content: prompt }
          ],
        }),
      });
      ...
      if (!response.ok) {
        ...
        console.error('Lovable AI error:', errorText);
        return new Response(
          JSON.stringify({ error: 'Erreur lors de la génération du tome' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
