...
async function callGemini(apiKey: string, sys: string, user: string, opts: { maxTokens?: number; temperature?: number } = {}) {
...
serve(async (req) => {
  ...
  try {
    const { title, subtitle, category, pages, format, targetAudience, language = 'fr', userApiKey} = await req.json();
    ...
    const content = await callGemini(userApiKey, 'Tu es un expert en tarification Amazon KDP. Réponds uniquement en JSON valide.', prompt);
    let parsed;
