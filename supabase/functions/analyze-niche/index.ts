...
serve(async (req) => {
  ...
  try {
    const { niche, userApiKey } = await req.json();
    ...
    const content = await callGemini(userApiKey, 'Tu es un expert en édition de livres numériques et marketing KDP Amazon. Réponds uniquement en JSON valide sans markdown.', prompt);
