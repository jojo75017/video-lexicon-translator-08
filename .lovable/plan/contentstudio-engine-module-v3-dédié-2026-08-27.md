...
Nouveau module `/v3/contentstudio` : on crée un livre conforme KDP, puis on le décline en un seul clic en formation vidéo (scripts, slides, voix off, sous-titres, **et MP4 monté par leçon**). Aucune clé API à fournir : tout passe par l'IA déjà intégrée à Ebookstudio.
...
3. **Onglet Formation vidéo** (après déblocage)
   - Pour chaque chapitre : titre vidéo, script en 3 blocs (accroche / cœur / passage à l'action), slides (titre, puces, visuel), voix off MP3, sous-titres SRT/VTT, durée estimée, **et MP4 monté**.
   - Export : pack ZIP par leçon (script .txt, slides .json, audio .mp3, sous-titres .srt, **vidéo .mp4**).
...
- `cs_video_lessons` (chapter_id, video_title, script_hook, script_core, script_action, slides_json, audio_url, subtitle_vtt_url, video_mp4_url, duration_seconds)
...
**Storage** : bucket privé `contentstudio` (couvertures, MP3, sous-titres, MP4) servi par URLs signées.

**Edge functions** (toutes via la passerelle Lovable AI, secret `LOVABLE_API_KEY` déjà en place — pas d'`OPENAI_API_KEY` ni de clé abonné) :
...
- `cs-video-voice` : voix off MP3 + sous-titres (ElevenLabs via `ELEVENLABS_API_KEY` côté serveur), stockés dans le bucket.
- `cs-video-render` : assemble les slides + voix off + sous-titres en un MP4 par leçon. Moteur server-side (Remotion ou ffmpeg via une edge function + stockage du MP4 rendu dans le bucket `contentstudio`). Lancé à la demande après génération de la voix off.

**Front** : `src/pages/v3public/ContentStudio*.tsx`, composants sous `src/components/v3/contentstudio/`, types dans `src/types/contentStudio.ts` (les interfaces `Project`, `Chapter`, `Slide`, `VideoLesson` que vous avez fournies, `video_mp4_url` en plus).
...
6. Voix off, sous-titres.
7. **Montage MP4 par leçon** (slides + voix off + sous-titres → MP4 rendu côté serveur, stocké dans le bucket, export ZIP final incluant le .mp4).
