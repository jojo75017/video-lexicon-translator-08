# Tarif unique 347 € + les 10 Spécialistes IA (EN ATTENTE — début septembre)

Deux volets en attente : le **nouveau tarif unique 347 €** et les **10 spécialistes IA**.
...
## Volet 1 — Tarif unique de lancement (347 €)
...
### Ce qui change dans l'application (à faire en septembre)
...
1. **Septembre** : tarif unique 347 € (données `v3Pricing.ts`, page de commande `/commander`, produits de paiement Stripe, textes, compte à rebours 31/12/2026).
2. **Septembre** : page `/v3/mise-a-niveau` à 97 € et marquage des anciens clients.
3. **Septembre** : les 10 spécialistes (données `aiSpecialists.ts`, écran `/v3/specialistes`, branchements modules).
4. **Septembre** : missions et contrôles de sortie par spécialiste, puis test réel sur Camille, Noémie et Prune.

## Bug de couverture en cours (séparé)

Le bug de génération de couverture signalé le 27/08 est identifié : le modèle `google/gemini-3.1-flash-image-preview` n'existe dans aucun catalogue (Lovable AI ni OpenRouter). Correction à appliquer dès que ce plan tariffier est validé ou mis de côté.
